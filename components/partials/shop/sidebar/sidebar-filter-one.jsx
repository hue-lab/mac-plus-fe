...
export default function SidebarFilterOne({ type = "left", isFeatured = false, filters = [] }) {
  ...
  const getPathname = () => {
    return typeof catalogueUrl === "string"
      ? router.pathname.replace("[...catalogue]", catalogueUrl)
      : router.pathname;
  };
  ...
  const filterByPrice = (filterPrice) => {
    ...
    for (let key in query) {
      if (key !== "min_price" && key !== "max_price" && key !== "page" && key !== "grid")
        ...
        {!loading && filters ? (
          <div className="sticky-sidebar">
              ...
              <div className="filter-actions mb-4">
                ...
                <ALink
                  href={{
                    ...
                    query: { grid: query.grid, type: router.query.type ? router.query.type : null },
                  }}
                  scroll={false}
            ...
            <div className="widget widget-collapsible">
              <Card
                title="<span class='widget-title d-block text-dark'>Цена<span class='toggle-btn p-0 parse-content'></span></span>"
                type="parse"
                expanded={true}
              >
              ...
              filters.map((item, index) =>
                ["CHECKBOX"].includes(item.type) ? (
                  <div key={index} className="widget widget-box-checkbox widget-collapsible">
                    <Card type="parse" expanded={true}>
                      <ul className="filter-items">
                        <li
                          className={
                            containsAttrInUrl(item.code || item._id, "true") ? "active" : ""
                          }
                          key={index}
                        >
                          <ALink
                            ...
                            href={{
                              ...
                              query: {
                                ...query,
                                page: 1,
                                [item.code || item._id]: !containsAttrInUrl(
                                  item.code || item._id,
                                  "true"
                                )
                                  ? true
                  ...
                  <div key={index} className="widget widget-collapsible">
                    <Card
                      title={`<span class='widget-title d-block text-dark'>${item.name}<span class='toggle-btn p-0 parse-content'></span></span>`}
                      type="parse"
                      expanded={false}
                    >
                      {item.type === "NUMBER_SELECT" && (
                        <ul className="widget-body filter-items">
                          {(item.options || []).map((option, index) => (
                            <li
                              className={
                                containsAttrInUrl(item.code || item._id, option) ? "active" : ""
                              }
                              key={index}
                            >
                              <ALink
                                ...
                                href={{
                                  ...
                                  query: {
                                    ...query,
                                    page: 1,
                                    [item.code || item._id]: getUrlForAttrs(
                                      item.code || item._id,
                                      option
                                    ),
                                  },
